      ...
      <div className="space-y-8">
        ...
        <div className="scroll-mt-20 space-y-3" id="param-useCase">
          ...
          <p className="text-sm text-muted-foreground">
            Select models by use case. Pass one or more as a comma-separated list: <code className="bg-muted px-1.5 py-0.5 rounded text-sm">?useCase=vision,tools</code>
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="text-muted-foreground">
                {USE_CASE_DEFINITIONS.filter(uc => uc.key !== 'all').map((useCase, index, arr) => (
        ...
        <div className="scroll-mt-20 space-y-3" id="param-topN">
          ...
          <p className="text-sm text-muted-foreground">
            Return only the top N models based on sort order. Range: {TOP_N_MIN}-{TOP_N_MAX}. Default: unlimited. Example: <code className="bg-muted px-1 py-0.5 rounded text-sm">?topN=10</code>
          </p>
        </div>
        ...
        <div className="scroll-mt-20 space-y-3" id="param-maxErrorRate">
          ...
          <p className="text-sm text-muted-foreground">
            Exclude models with error rate above this percentage ({MAX_ERROR_RATE_MIN}-{MAX_ERROR_RATE_MAX}). Error rate = errors / (errors + successes). Example: <code className="bg-muted px-1 py-0.5 rounded text-sm">?maxErrorRate=20</code> excludes models with more than 20% error rate.
          </p>
        </div>
        ...
        <div className="scroll-mt-20 space-y-3" id="param-timeRange">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            . Default: <code className="bg-muted px-1.5 py-0.5 rounded text-sm">{DEFAULT_TIME_RANGE}</code>.
          </p>
        </div>
        ...
        <div className="scroll-mt-20 space-y-3" id="param-myReports">
          ...
          <p className="text-sm text-muted-foreground">
            When set to true, calculate error rates from only your own reported issues instead of all community reports. Requires API key authentication. Default: false. Example: <code className="bg-muted px-1 py-0.5 rounded text-sm">?myReports=true</code>
          </p>
        </div>
      </div>
    </section>
