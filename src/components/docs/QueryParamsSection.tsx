...
export function QueryParamsSection() {
  return (
    <section id="query-params" className="mt-20 scroll-mt-20">
      <h2 className="mb-4 type-heading">Query Parameters</h2>
      ...
      <div className="space-y-8">
        ...
        <div className="scroll-mt-20 space-y-3" id="param-useCase">
          <h3 className="type-title">useCase</h3>
          <p className="type-label text-muted-foreground">
            Select models by use case. Pass one or more as a comma-separated list:{' '}
            <code className="bg-muted px-1.5 py-0.5 rounded type-label">?useCase=vision,tools</code>
          </p>
          <div className="overflow-x-auto">
            <table className="w-full type-label">
              <thead>
                <tr className="border-b">
                  <th className="py-2 pr-4 text-left type-label">Value</th>
                ...
                {USE_CASE_DEFINITIONS.filter((uc) => uc.key !== 'all').map(
                  (useCase, index, arr) => (
                    <tr key={useCase.key} className={index === arr.length - 1 ? '' : 'border-b'}>
                      <td className="py-2 pr-4 font-mono type-caption">{useCase.key}</td>
        ...
        <div className="scroll-mt-20 space-y-3" id="param-sort">
          <h3 className="type-title">sort</h3>
          <p className="type-label text-muted-foreground">
            Control the order models are returned. This determines fallback priority when iterating
            through the list. Example:{' '}
            <code className="bg-muted px-1.5 py-0.5 rounded type-label">?sort=contextLength</code>
          </p>
          <div className="overflow-x-auto">
            <table className="w-full type-label">
              <thead>
                <tr className="border-b">
                  <th className="py-2 pr-4 text-left type-label">Value</th>
                  <th className="py-2 pr-4 text-left type-label">Label</th>
                  <th className="py-2 text-left type-label">Description</th>
                </tr>
              </thead>
              <tbody className="text-muted-foreground">
                {SORT_DEFINITIONS.map((sort, index) => (
                  ...
                  >
                    <td className="py-2 pr-4 font-mono type-caption">{sort.key}</td>
        ...
        <div className="scroll-mt-20 space-y-3" id="param-maxErrorRate">
          ...
          <p className="type-label text-muted-foreground">
            ...
            <code className="bg-muted px-1 py-0.5 rounded type-label">?maxErrorRate=20</code>{' '}
            excludes models with more than 20% error rate.
          </p>
        </div>

        {/* timeRange parameter */}
        <div className="scroll-mt-20 space-y-3" id="param-timeRange">
          <h3 className="type-title">timeRange</h3>
          <p className="type-label text-muted-foreground">
            Time window for calculating error rates. Options:{' '}
            {TIME_RANGE_DEFINITIONS.map((tr, index) => (
              <span key={tr.value}>
                {index > 0 && ', '}
                <code className="bg-muted px-1.5 py-0.5 rounded type-label">{tr.value}</code>
              </span>
            ))}
            . Default:{' '}
            <code className="bg-muted px-1.5 py-0.5 rounded type-label">{DEFAULT_TIME_RANGE}</code>.
          </p>
        </div>

        {/* myReports parameter */}
        <div className="scroll-mt-20 space-y-3" id="param-myReports">
          <h3 className="type-title">myReports</h3>
          <p className="type-label text-muted-foreground">
            ...
            <code className="bg-muted px-1 py-0.5 rounded type-label">?myReports=true</code>
          </p>
        </div>
      </div>
    </section>
