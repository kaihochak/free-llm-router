...
describe('/api/v1/models/ids', () => {
  it('returns 401 without Authorization header', async () => {
    const { GET } = await import('../../src/pages/api/v1/models/ids.ts');

    const context = createMockContext({
      url: 'http://localhost:4321/api/v1/models/ids',
      locals: {
        runtime: {
          env: {
  ...
  it('returns 204 for OPTIONS request (CORS preflight)', async () => {
    const { OPTIONS } = await import('../../src/pages/api/v1/models/ids.ts');
